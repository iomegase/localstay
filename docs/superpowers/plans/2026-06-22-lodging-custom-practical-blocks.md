...
### Task 9: Réordonnancement des blocs par glisser-déposer (dnd-kit)

**Files:**
- Modify: `src/features/guide-customization/lib/validation.ts` (helper `reorderById`)
- Modify: `src/features/guide-customization/components/PracticalBlocksEditor.tsx` (dnd-kit)
- Test: `tests/unit/guide-customization.reorder-by-id.test.ts`

**Interfaces:**
- Consumes: `reorderById` (ce task), `@dnd-kit/core`, `@dnd-kit/sortable`, `@dnd-kit/utilities` (déjà dépendances du projet, cf. `CustomizationForm`).
- Produces: `reorderById<T extends { id?: string }>(items: T[], activeId: string, overId: string): T[]` ; `PracticalBlocksEditor` réordonnable au glisser-déposer (poignée par bloc), chaque bloc reçoit un `id` client stable.

- [ ] **Step 1: Écrire le test du helper (échoue)**

Créer `tests/unit/guide-customization.reorder-by-id.test.ts` :
...
Run: `npx jest tests/unit/guide-customization.reorder-by-id.test.ts`
Expected: FAIL — `reorderById is not a function`.

- [ ] **Step 3: Implémenter le helper pur**

Dans `src/features/guide-customization/lib/validation.ts`, à la fin du fichier :
...
- [ ] **Step 5: Réécrire `PracticalBlocksEditor` avec dnd-kit**

Remplacer intégralement le contenu de `src/features/guide-customization/components/PracticalBlocksEditor.tsx` par cette version (poignée de drag par bloc, `id` client stable assigné à l'ajout) :
...
- [ ] **Step 6: Lancer les tests éditeur (non-régression Task 7 + dnd)**

Run: `npx jest tests/unit/guide-customization.practical-blocks-editor.test.tsx tests/unit/guide-customization.reorder-by-id.test.ts`
Expected: PASS. (Le test Task 7 reste vert : ajouter un `id` au bloc ne casse pas les assertions `toContain`/`toBe('[]')`.)
...
```bash
git add src/features/guide-customization/lib/validation.ts \
        src/features/guide-customization/components/PracticalBlocksEditor.tsx \
        tests/unit/guide-customization.reorder-by-id.test.ts
git commit -m "feat(guide-customization): drag-and-drop reordering of practical blocks
...
- Réordonnancement entre blocs par **glisser-déposer (dnd-kit)** → Task 9 ; `sort_order` réindexé par position à l'enregistrement (Task 3/5). ✅
...
**Type consistency :** `PracticalBlockInput`/`PracticalBlockResponse` (Task 2) sont utilisés tels quels par Tasks 3–9. `normalizePracticalBlocks` (Task 3) renvoie `NormalizedPracticalBlock` consommé en Task 5. `PracticalBlocksEditor` (Task 7) a la signature `{ value, onChange, lodgingId }` consommée en Task 8. `reorderById` (Task 9) est ajouté à `validation.ts` et consommé par l'éditeur. Task 9 réécrit `PracticalBlocksEditor` en assignant un `id` client à chaque bloc ; l'API (Task 4) accepte un `id` string optionnel ignoré à la persistance.
