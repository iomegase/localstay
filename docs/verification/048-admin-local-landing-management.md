...
three reviews with known destination slugs. The first post-backfill integrity
query was rejected by the tool's usage limit. After access resumed, the query
confirmed four destinations with exactly three distinct intents each, all three
reviews attached and zero null review `destination_id` values. Saint-Gervais
and Saint-Nicolas are active; Megève and Combloux remain inactive.

The follow-up migration
`20260912095500_require_local_landing_review_destination` was generated by an
offline diff of the nullable and final Prisma schemas, reviewed, and applied
with `prisma migrate deploy` (exit 0). It replaces the optional foreign key and
sets `LocalLandingReview.destination_id` to NOT NULL. **The configured database
migration is complete, but the final branch has not yet been merged or deployed.**
Post-enforcement checks: `prisma migrate status` reports all 31 migrations
applied; `npm run build` completed with exit 0 against the prepared database,
