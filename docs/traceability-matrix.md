...
| AC-04-01 | Favoris locaux → cards verticales compactes sans accordéon + ouverture en panneau modal avec backdrop, sans navigation d'URL + footer actions modal en pilules alignées sur une ligne, sans réservation, y compris sur la variante contextualisée `/guide/[city-slug]/mes-favoris` | `src/app/(public)/mes-favoris/page.tsx`<br>`src/app/(public)/guide/[city-slug]/mes-favoris/page.tsx`<br>`src/features/public-menu/components/FavoritesList.tsx`<br>`src/features/public-menu/components/FavoritePoiModal.tsx`<br>`src/features/categories/components/ActionButtons.tsx` | `tests/integration/favorites.modal.test.tsx`<br>`tests/integration/favorite-poi-modal.shell.test.tsx`<br>`tests/integration/city-guide.contextual-favorites-page.test.tsx`<br>`tests/unit/categories.AC-01-02-03.action-buttons.test.tsx` | ✅ done |
| BR-09 | Nom produit MyStay + libellés publics `Bienvenue`, `Vos favoris`, `Les recommandations de {owner.name}` + bouton `Guide` contextuel sur `/guide/[city-slug]` + routes contextualisées `Vos favoris`/`Contact` conservées hors remplacement blog 029 + item actif `#bd9254` + items inactifs `#6f7480` + bottom nav complet transparent pendant le scroll puis visible à l'arrêt | `src/app/(public)/layout.tsx`<br>`src/features/city-guide/components/PublicBottomNav.tsx`<br>`src/features/city-guide/components/PublicMenu.tsx`<br>`src/features/city-guide/lib/public-paths.ts`<br>`src/features/public-menu/lib/lodging-mode.ts` | `tests/unit/public-layout.mockup-menu.test.tsx`<br>`tests/unit/public-bottom-nav.test.tsx`<br>`tests/unit/weather.public-menu.test.tsx` | ✅ done |
...
| AC-01-02 | Card affiche tous les champs requis + première photo exploitable centrée, `object-cover` en portrait/carré et `object-contain` en paysage, jamais logo/placeholder + actions rectangulaires noir/blanc alignées dans le panneau détaillé | `src/features/categories/components/PoiCard.tsx`<br>`src/features/categories/lib/poi-header-image-fit.ts`<br>`src/features/categories/lib/photo-url.ts`<br>`src/features/categories/queries/poi-cards.ts`<br>`src/features/categories/queries/all-poi-cards.ts` | `tests/integration/categories.AC-01-02.poi-card-renders.test.tsx`<br>`tests/unit/categories.poi-header-image-fit.test.ts`<br>`tests/unit/categories.AC-01-01-02-01.poi-sorting.test.ts`<br>`tests/unit/guide-customization.BR-12.all-poi-guide.test.ts` | ✅ done |
...
| AC-04-01 | Entrée `Logements` visible dans le menu burger d'une City | `src/features/city-guide/components/PublicMenu.tsx` | `tests/unit/weather.public-menu.test.tsx`<br>`tests/unit/public-layout.mockup-menu.test.tsx` | ✅ done |
...
| AC-04-03 | Entrée `Logements` du menu redirige vers la liste dédiée | `src/features/city-guide/components/PublicMenu.tsx` | `tests/unit/weather.public-menu.test.tsx`<br>`tests/unit/public-layout.mockup-menu.test.tsx` | ✅ done |
...
| AC-06-01 | Admin liste et filtre les fiches logement | `src/app/admin/lodgings/page.tsx`<br>`src/features/lodging-showcase/components/AdminLodgingProfilesTable.tsx`<br>`src/app/api/admin/lodgings/public-profiles/route.ts`<br>`src/features/lodging-showcase/queries/admin-public-profiles.ts` | `tests/integration/lodging-showcase.admin-page.test.tsx`<br>`tests/contract/lodging-showcase.admin-api.test.ts` | ✅ done |
