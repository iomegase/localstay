...
| AC-04-01 | Favoris locaux → cards verticales compactes sans accordéon + ouverture en panneau modal avec backdrop, sans navigation d'URL | `src/app/(public)/mes-favoris/page.tsx`<br>`src/features/public-menu/components/FavoritesList.tsx`<br>`src/features/public-menu/components/FavoritePoiModal.tsx` | `tests/integration/favorites.modal.test.tsx`<br>`tests/integration/favorite-poi-modal.shell.test.tsx` | ✅ done |
...
| AC-01-02 | Card affiche tous les champs requis + première photo exploitable centrée, `object-cover` en portrait/carré et `object-contain` en paysage, jamais logo/placeholder | `src/features/categories/components/PoiCard.tsx`<br>`src/features/categories/lib/poi-header-image-fit.ts`<br>`src/features/categories/lib/photo-url.ts`<br>`src/features/categories/queries/poi-cards.ts`<br>`src/features/categories/queries/all-poi-cards.ts` | `tests/integration/categories.AC-01-02.poi-card-renders.test.tsx`<br>`tests/unit/categories.poi-header-image-fit.test.ts`<br>`tests/unit/categories.AC-01-01-02-01.poi-sorting.test.ts`<br>`tests/unit/guide-customization.BR-12.all-poi-guide.test.ts` | ✅ done |
