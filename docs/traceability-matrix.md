...
| AC-01-02 | Card affiche tous les champs requis + première photo exploitable en `cover` centré, jamais logo/placeholder/`contain` | `src/features/categories/components/PoiCard.tsx`<br>`src/features/categories/lib/photo-url.ts`<br>`src/features/categories/queries/poi-cards.ts`<br>`src/features/categories/queries/all-poi-cards.ts` | `tests/integration/categories.AC-01-02.poi-card-renders.test.tsx`<br>`tests/unit/categories.AC-01-01-02-01.poi-sorting.test.ts`<br>`tests/unit/guide-customization.BR-12.all-poi-guide.test.ts` | ✅ done |
