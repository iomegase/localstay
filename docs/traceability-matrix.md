...
| AC-05-01/AC-05-02/AC-05-04/BR-18/BR-20 | Route interne de sync protégée, validation Zod, cron centralisé et première synchro réelle Google Search Console | `src/app/api/internal/analytics/sync/route.ts`<br>`src/features/admin-analytics/services/sync.ts`<br>`src/features/admin-analytics/services/google-search-console.ts`<br>`src/features/admin-analytics/schemas.ts`<br>`vercel.json` | `tests/contract/admin-analytics.sync-api.test.ts`<br>`tests/unit/admin-analytics.gsc-sync.test.ts` | ✅ done |
