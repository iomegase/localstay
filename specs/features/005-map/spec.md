...
- En mode séjour actif, si le Lodging possède des coordonnées géocodées, `/map` affiche un pin logement discret et pulsant, visuellement distinct des markers POI et sans interaction popup. Au survol ou focus du pin logement, une mini-card blanche affiche la photo du logement si disponible et le nom du Lodging.
- Le sélecteur de fond de carte `Plan / IGN / Satellite` est affiché comme un contrôle segmenté lisible : l'option active possède un contraste texte/fond explicite, et les états hover restent limités aux options inactives.
...
| AC-04-01 | Side menu catégories sur `/map` avec "Tous", icône, couleur, compteur, pin logement et mini-card photo si coordonnées disponibles | unit |
