...
- **BR-09**: Le nom produit public est MyStay. Les libellés de navigation validés le 2026-05-21 et ajustés le 2026-07-10 sont : bottom nav `Explorer`/`Bienvenue`/`Nos recommandations` → `Coup de coeur` rendu sur deux lignes avec une icône coeur rouge, `Logement` → `Guide logement` rendu sur deux lignes, bouton `Guide` visible uniquement dans le contexte anonyme `/guide/[city-slug]` et pointant vers la racine de cette ville ; en mode logement, le lien redondant `Guide` n'est pas rendu dans la bottom nav. `Vos favoris` n'est plus rendu dans la bottom nav et est exposé dans le menu burger. Quand une City est active dans l'URL publique, les entrées `Vos favoris` et `Contact` utilisent des routes contextualisées sous `/guide/[city-slug]/...` afin de conserver le contexte ville ; menu burger `Home` → `Bienvenue`, `Logements` → `Tous nos logements`; menu burger `Services Privés` → `Les recommandations de {owner.first_name}` si le prénom Owner est connu, sinon `Les recommandations de votre hôte`. L'item sélectionné du bottom nav public utilise la couleur `#bd9254`; les items inactifs utilisent un gris lisible `#6f7480` sur la surface glassmorphism et un hover gris clair `#9ca3af`, sans hover rose ; l'item GPS du bottom nav est rouge par défaut, orange pendant l'acquisition et vert quand la position est active ; le bottom nav complet (surface, textes et icônes) devient transparent pendant que l'utilisateur scrolle, puis revient visible en glassmorphism quand le scroll s'arrête.
...
- **Success state** : home Guide avec intitulé de guide, sous-titre, champ de recherche visuel, ligne horizontale de catégories, vue "Tous les POI" avec infinite scroll, bottom navigation incluant `Coup de coeur` avec coeur rouge, `Guide logement` en contexte logement, `Guide` uniquement en contexte guide anonyme vers `/guide/[city-slug]`, mais pas `Vos favoris`, item actif en `#bd9254`, items inactifs en `#6f7480` avec hover gris clair, item GPS rouge/orange/vert selon l'état de géolocalisation et menu entièrement transparent pendant le scroll puis visible à l'arrêt du scroll. L'entrée `Vos favoris` est disponible dans le menu burger et l'entrée logements y est libellée `Tous nos logements`.
...
- Dans ce panneau modal, le footer d'actions reprend le style des actions de fiche POI récentes : pilules blanches compactes, ombre douce, pastille icône colorée, libellé uppercase en 9px (`Appeler`, `Site web`, `Itinéraire`) et alignement sur une seule ligne sans scroll horizontal. Il est fixé en bas de l'écran (`bottom` avec marge légère), disparaît pendant le scroll du panneau modal, puis revient visible quand le scroll s'arrête. Les sous-libellés `Téléphone`, `Web` et `Maps` ne sont pas rendus. `Réserver` n'est pas rendu en MVP 1.
...
| BR-09 | Navigation publique : libellés validés, `Coup de coeur` avec coeur rouge et `Guide logement` sur deux lignes, burger `Tous nos logements`, bouton `Guide` contextuel anonyme et absent en mode logement, `Vos favoris` dans le menu burger et absent de la bottom nav, item actif `#bd9254`, items inactifs `#6f7480` avec hover gris clair, GPS rouge/orange/vert selon son état, bottom nav entièrement transparent pendant le scroll puis visible à l'arrêt | unit |
