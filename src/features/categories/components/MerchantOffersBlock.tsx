    ...
    <section className="space-y-3" data-testid="merchant-offers">
      ...
      <div className="space-y-3">
        {activeOffers.map(offer => (
          <article key={offer.id} className="rounded-3xl border border-gold/25 bg-white p-4 shadow-sm">
            <div className="flex items-start gap-3">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gold/10 text-gold">
                <BadgePercent className="h-5 w-5" />
              </span>
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-gold">Offre spéciale</p>
