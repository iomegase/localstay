...
function PrimaryCta({ content }: { content: LocalServiceContent }) {
  if (content.ctaHref.startsWith('mailto:')) {
    return (
      <a className={marketingPrimaryButtonClass} href={content.ctaHref}>
        {content.ctaLabel}
  ...
  return (
    <Link className={marketingPrimaryButtonClass} href={content.ctaHref}>
      {content.ctaLabel}
      <ArrowRight aria-hidden="true" className="ml-3 h-4 w-4" />
    </Link>
  )
...
export function LocalServiceLanding({
  ...
  const content = destination.services[intent]
  const isConcierge = intent === 'concierge'
          ...
          <div className="rounded-[28px] bg-slate-50 px-6 py-10 sm:px-10 sm:py-12 lg:flex lg:items-end lg:justify-between lg:gap-12">
            <div>
              ...
              <h2 className="max-w-[650px] text-[30px] font-bold leading-[1.12] tracking-[-0.04em] text-slate-900 sm:text-[38px]">
                {isConcierge
                  ? `Parlons de votre logement à ${destination.name}.`
                  : `Préparons votre séminaire à ${destination.name}.`}
              </h2>
