  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-amber-300">Pipeline randonnée</p>
          <h1 className="mt-2 text-3xl font-semibold text-white">Acquisition randonnées</h1>
      ...
      <div className="grid gap-4">
        {runs.length === 0 && (
          <Card className="border-white/10 bg-white/5 text-slate-100">
            <CardContent className="p-6 text-sm text-slate-400">Aucun run randonnée pour le moment.</CardContent>
          </Card>
        )}
        {runs.map(run => (
          <Card key={run.id} className="border-white/10 bg-white/5 text-slate-100">
            <CardHeader className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
              <div>
                ...
                <p className="mt-1 text-xs text-slate-500">Sources: {run.source_types.join(', ')}</p>
              </div>
