  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-4">
        ...
        <span className="text-xs font-bold text-blue-600">{checked.size} / {items.length}</span>
      </div>
      <progress
      ...
      <div className="mt-3 divide-y divide-slate-100">
        {items.map((item, index) => (
          <label key={`${item}-${index}`} className="flex cursor-pointer items-start gap-3 py-3 text-sm text-slate-600">
            <input
              type="checkbox"
            ...
            <span className="pt-0.5 peer-checked:text-slate-400 peer-checked:line-through">{item}</span>
          </label>
        ))}
      </div>
    </div>
