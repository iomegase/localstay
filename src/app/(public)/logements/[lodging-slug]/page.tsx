...
interface Props {
  params: Promise<{ 'lodging-slug': string }>
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { 'lodging-slug': lodgingSlug } = await params
  const detail = await getPublishedLodgingDetailBySlug(lodgingSlug)
  if (!detail) {
    return {
...
export default async function LodgingDetailPage({ params }: Props) {
  ...
  const breadcrumb = breadcrumbSchema([
    ...
    { name: detail.title, path: detailPath },
  ])
