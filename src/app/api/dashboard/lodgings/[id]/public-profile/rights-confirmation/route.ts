...
import { OwnerRightsConfirmationSchema } from '@/features/lodging-showcase/schemas'
import { confirmContentRights } from '@/features/lodging-showcase/queries/owner-public-profile'
...
export async function POST(
  ...
  const parsed = OwnerRightsConfirmationSchema.safeParse(body)
  ...
  const payload = await confirmContentRights(session.owner.id, id, parsed.data.statement_version)
